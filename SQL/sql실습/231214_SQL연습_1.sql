...
ON DELETE NO ACTION ON UPDATE NO ACTION; # 2개의 옵션 : 
...
DESC EMP;
INSERT into emp VALUES(1, '신짱구', '유치원생', 2, now(), 800, Null, 70); # => fk 존재 하지 않음

-- 신짱구 40번 부서로 넣고 50으로 변경, 삭제
select * from dept;
select * from emp;
desc emp;
insert into emp values(null, '신짱구', '유치원생', 2, now(), 800, Null, 40);
...
show databases; # 우리 눈에는 보이지 않지만 숨겨진 데이터베이스가 있음! => information_schema
show tables;
...
WHERE TABLE_NAME IN ('EMP01'); # 다른 컬럼에 영향을 미치는 컬럼에 대한 관리
...
SELECT *
FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
WHERE TABLE_NAME IN ('EMP01'); # 다른 테이블, 컬럼에 영향을 미치는 조건만 관리

desc emp01;

-- 4. drop 후 dictionary table 검색
# alter 테이블명 drop 조건 컬럼명;
alter table emp01 drop empno not null; 
alter table emp01 modify empno varchar(10); -- null 
-- 9.integrity.sql
-- DB 자체적으로 강제적인 제약 사항 설정
DROP DATABASE IF EXISTS fisa;
CREATE DATABASE fisa DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
alter table emp04 AUTO_INCREMENT=1; -- 이미 있는 값보다 큰 값을 지정해줘야 걸린다.
...
-- 16. drop a check : 제약조건명 검색 후에 이름으로 삭제
